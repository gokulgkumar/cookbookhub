...
def signupPage(request):
    if request.method=='POST':
        ...
        if form.is_valid():
            form.save()
            message=messages.success(request,'Registration Successful, continue to Login')
...
def search_app(request):
    ...
    if request.method == 'GET':
        ...
        if search_app:
            ...
            search=Recipes.objects.filter(Q(recipe_name__icontains =search_app ) | Q(user__first_name__icontains =search_app) | Q(user__last_name__icontains = search_app) )
...
def adding_recipe(request):
    if request.method=='POST':
        ...
        saverecipe=Recipes(user=user,recipe_name=recipename,category=category,incredients=incredients,nutrients=nutrients,recipe=recipe,image=image)
        saverecipe.save()
